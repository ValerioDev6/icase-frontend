...
export class ReportesPdfService {
	...
	dowLoadVentaTicket(id: string): Observable<Blob> {
		return this._httpClient
			.get(`${this.API_URL}/reports-pdf/ventas-ticket/${id}`, {
				responseType: 'blob',
			})
			.pipe(catchError(this.handleError));
	}
	...
	dowloadKardexPdf(): Observable<Blob> {
		return this._httpClient.get(`${this.API_URL}/reports-pdf/kardex`, {
			responseType: 'blob',
		});
	}

	dowloadClientrePdf(): Observable<Blob> {
		return this._httpClient.get(`${this.API_URL}/reports-pdf/clientes`, {
			responseType: 'blob',
		});
	}

	dowloadPersonalPdf(): Observable<Blob> {
		return this._httpClient.get(`${this.API_URL}/reports-pdf/personal`, {
			responseType: 'blob',
		});
	}
}
