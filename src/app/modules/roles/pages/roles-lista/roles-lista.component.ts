...
import { AgregarPermisosModalComponent } from '../../components/agregar-permisos-modal/agregar-permisos-modal.component';
import { PermisosService } from '../../../../core/services/permisos.service';
import { ActualizarPermisosComponent } from '../../components/actualizar-permisos/actualizar-permisos.component';
...
export default class RolesListaComponent implements OnInit {
	...
	openAgregarPermisosModal() {
		const modal = this._modal.create({
			nzTitle: ' Agregar Permisos',
			nzContent: AgregarPermisosModalComponent,
			nzFooter: null,
			nzWidth: '500px',
		});

		modal.afterClose.subscribe((resultado: boolean) => {
			if (resultado) {
				this.loadDataRoles();
			}
		});
	...
	openEditarPermisoModal(permiso: IPermisosResponse) {
		const modal = this._modal.create({
			nzTitle: 'Actualziar Permisos',
			nzContent: ActualizarPermisosComponent,
			nzData: { id_permiso: permiso.id_permiso },
			nzFooter: null,
		});
		modal.afterClose.subscribe((resultado: boolean) => {
			if (resultado) {
	...
	delelePermiso(id: string): void {
		Swal.fire({
			title: '¿Está seguro?',
			text: `Este proceso no es reversible, está a punto de eliminar su permiso , ${id}`,
			showCancelButton: true,
			confirmButtonText: 'Sí, eliminar',
			cancelButtonText: 'No, cancelar',
			customClass: {
			...
			if (result.isConfirmed) {
				...
				this.permisoService.deletePermiso(id).subscribe({
					next: () => {
						this.loadDataRoles();
						this.message.success('Permiso eliminada con éxito');
					},
					error: () => {
						this.loading = false;
						this.message.error('Error al eliminar el permiso');
					},
				});
			}
		});
