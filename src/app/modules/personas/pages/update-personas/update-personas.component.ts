...
@Component({
	selector: 'app-update-personas',
	standalone: true,
	imports: [FormsModule, ReactiveFormsModule, CommonModule, NZ_MODULES],
	templateUrl: './update-personas.component.html',
	styleUrl: './update-personas.component.scss',
})
export default class UpdatePersonasComponent implements OnInit {
	...
	ngOnInit(): void {
		this.activatedRoute.params.subscribe((params) => {
			...
			if (personaId) {
				this.loadPersonaData(personaId);
			}
		});
		this.loadData();
	}
	...
	private loadPersonaData(personaId: string) {
		this.isLoading = true;
		this.personasService.findOnePersona(personaId).subscribe({
			next: (persona) => {
				this.persona = persona;
				// Actualizamos el formulario con los datos de la persona
				this.personaForm.patchValue({
					...
					id_tipo_telefono: persona.id_tipo_telefono,
					numero_documento: persona.numero_documento,
					telefono: persona.telefono,
					razon_social: persona.razon_social,
					estado_documento: persona.estado_documento,
					condicion_documento: persona.condicion_documento,
			...
			error: (error) => {
				this.message.error(`Error al cargar los datos de la persona: ${error}`);
				this.isLoading = false;
			},
		});
	}
	...
	onSubmit() {
		if (this.personaForm.valid) {
			...
			this.personasService.updatePersonasGeneral(this.persona.id_persona, personaData).subscribe({
				next: () => {
					this.message.success('Persona actualizado Exitosamente');
					this.resetForm();
				},
